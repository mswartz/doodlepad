<template>
        <div class="notebook">
            ...
            <ul>
                <li v-for="(page, index) of filterList" class="page" v-bind:class="{ 'active': index === activePage }" @click="changePage(index)" v-bind:key="index">
                    <div>{{page.title}}</div>
                </li>
        ...
        computed: {
            filteredList: function(){
                return this.pages.filter((page)=>{
                    return page.title.match(this.search);
                });
            }
        }
      }
